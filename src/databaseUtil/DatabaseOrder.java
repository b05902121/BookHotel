...
public class DatabaseOrder extends DatabaseConnect {
    ...
    public boolean updateOrder(Order order) {
        ...
        /* Update Room table */
        ResultSet resultSet = null;
        Integer startDateOld, endDateOld;
        Integer startDateNew = order.getStartDate(), endDateNew = order.getEndDate();
        Integer sNumOld, dNumOld, qNumOld;
        Integer sNumNew = order.getsNum(), dNumNew = order.getdNum(), qNumNew = order.getqNum();
        try {
            prstmtRoom = conn.prepareStatement("SELECT * FROM `Orders` WHERE `OrderID`=?;");
            prstmtRoom.setInt(1, order.getOrderId());
            resultSet = prstmtRoom.executeQuery();
            resultSet.next();
            startDateOld = resultSet.getInt("StartDate");
            endDateOld = resultSet.getInt("EndDate");
            sNumOld = resultSet.getInt("SNum");
            dNumOld = resultSet.getInt("DNum");
            qNumOld = resultSet.getInt("QNum");
            if (resultSet.getInt("HotelID") != order.getHotelId()) {
                return false;
            }
            if (startDateOld > startDateNew || endDateOld < endDateNew) {
                return false;
            }
            if (sNumOld < sNumNew || dNumOld < dNumNew || qNumOld < qNumNew) {
                return false;
            }
            /* select from rooms */
            prstmtRoom = conn.prepareStatement("SELECT `RoomID`,`RoomType`,`DateIsAvailable` FROM `Rooms` WHERE `HotelID`=?;");
            ...
            while(resultSet.next()) {
                byte[] dateIsAvailableBytes = resultSet.getBytes("DateIsAvailable");
                String roomType = resultSet.getString("RoomType");
                if(sNumOld == 0 && dNumOld == 0 && qNumOld == 0) {
                    break;
                }
                else if(roomType.equals("Single") && sNumNew > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    sNumOld --;
                    sNumNew --;
                ...
                else if(roomType.equals("Double") && dNumNew > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    dNumOld --;
                    dNumNew --;
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)1, startDateOld, endDateOld);
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)0, startDateNew, endDateNew);
                ...
                else if(roomType.equals("Quad") && qNumNew > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    qNumOld --;
                    qNumNew --;
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)1, startDateOld, endDateOld);
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)0, startDateNew, endDateNew);
                ...
                else if(roomType.equals("Single") && sNumOld > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    sNumOld --;
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)1, startDateOld, endDateOld);
                    prstmtRoom.setBytes(1, dateIsAvailableBytes);
                    prstmtRoom.setInt(2, resultSet.getInt("RoomID"));
                ...
                else if(roomType.equals("Double") && dNumOld > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    dNumOld --;
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)1, startDateOld, endDateOld);
                    prstmtRoom.setBytes(1, dateIsAvailableBytes);
                    prstmtRoom.setInt(2, resultSet.getInt("RoomID"));
                ...
                else if(roomType.equals("Quad") && qNumOld > 0 && match(dateIsAvailableBytes, (byte)0, startDateOld, endDateOld)) {
                    qNumOld --;
                    dateIsAvailableBytes = setBit(dateIsAvailableBytes, (byte)1, startDateOld, endDateOld);
                    prstmtRoom.setBytes(1, dateIsAvailableBytes);
                    prstmtRoom.setInt(2, resultSet.getInt("RoomID"));
